...
operation_log = {} # in-memory store for storing the operation log
...
def log_operation(key, value, operation):
    if SOCKET_ADDRESS == "10.10.0.2:8090":
        operation_log[vector_clock[0]] = [key, value, operation]
    elif SOCKET_ADDRESS == "10.10.0.3:8090":
        operation_log[vector_clock[1]] = [key, value, operation]
    elif SOCKET_ADDRESS == "10.10.0.4:8090":
        operation_log[vector_clock[2]] = [key, value, operation]
...
def recover():
    if SOCKET_ADDRESS == "10.10.0.2:8090":
        if vector_clock[0] == 0:
            ...
            for i in range(1, vector_clock[0]+1):
                try:
                    key = operation_log[str(i)][0]
                    value = operation_log[str(i)][1]
                    operation = operation_log[str(i)][2]
                    if operation == 'PUT':
                        kv_store[key] = value
        ...
        if vector_clock[1] == 0:
            ...
            for i in range(1, vector_clock[1]+1):
                try:
                    key = operation_log[str(i)][0]
                    value = operation_log[str(i)][1]
                    operation = operation_log[str(i)][2]
        ...
        if vector_clock[2] == 0:
            ...
            for i in range(1, vector_clock[2]+1):
                try:
                    key = operation_log[str(i)][0]
                    value = operation_log[str(i)][1]
                    operation = operation_log[str(i)][2]
...
for view in views:
    if view != SOCKET_ADDRESS:
        ...
        try:
            ...
            sa_store[view] = True
            r = requests.get(f"http://{view}/recover")
            operation_log = r.json()["operation_log"]
            recover()
        ...
        for replica in temp_sa_store:
            if replica != SOCKET_ADDRESS:
                try:
                    ...
                    requests.delete(url, json={"socket-address": replica, "on-operation": "PUT", "key": key, "value": value})
                    # try again
...
def get_operation_log():
    if request.method == 'GET':
        return jsonify({"operation_log": operation_log, "causal-metadata": vector_clock}), 200
